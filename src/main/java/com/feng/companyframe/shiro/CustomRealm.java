...
public class CustomRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
        ...
        * 返回该用户的角色信息给授权器
        * */
        if (null != claims.get(Constants.JWT_ROLES_KEY)){
            Collection<String> roles = (Collection<String>) claims.get(Constants.JWT_ROLES_KEY);
            info.addRoles(roles);
        }
        /*
        * 返回该用户的权限信息给授权器
        * */
        if (null!= claims.get(Constants.JWT_PERMISSIONS_KEY)){
            Collection<String> permissions = (Collection<String>) claims.get(Constants.JWT_PERMISSIONS_KEY);
